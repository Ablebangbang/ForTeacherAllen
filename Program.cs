...
namespace ConsoleApp2
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //-----------------------------------------------------------------------------------------------------------
            //倒正triangle
            Func<int, int, string> funcCenter = (rows, currentRow) => currentRow <= rows ? new string(' ', currentRow - 1) + new string('*', 2 * (rows - currentRow) + 1) : string.Empty;


            //-------------------------------------------------------------------------------------------------------------
            //倒左triangle
            Func<int, int, string> funcLeft = (rows, currentRow) => currentRow <= rows ? new string('*', rows - currentRow + 1) : string.Empty;

            //--------------------------------------------------------------------------------------------------------------
            //倒右triangle
            Func<int, int, string> funcRight = (rows, currentRow) => currentRow <= rows ? new string('*', rows - currentRow + 1).PadLeft(rows) : string.Empty;
